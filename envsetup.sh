...
EOF
    ...
    local A=""
    local i
    for i in `cat $T/build/envsetup.sh $T/vendor/os/build/envsetup.sh | sed -n "/^[[:blank:]]*function /s/function \([a-z_]*\).*/\1/p" | sort | uniq`; do
      A="$A $i"
    done
    echo $A
}
...
function build_build_var_cache()
{
    ...
    cached_vars=(`cat $T/build/envsetup.sh $T/vendor/os/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
    cached_abs_vars=(`cat $T/build/envsetup.sh $T/vendor/os/build/envsetup.sh | tr '()' '  ' | awk '{for(i=1;i<=NF;i++) if($i~/get_abs_build_var/) print $(i+1)}' | sort -u | tr '\n' ' '`)
...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^eunoia_") ; then
        EUNOIA_BUILD=$(echo -n $1 | sed -e 's/^eunoia_//g')
    else
        EUNOIA_BUILD=
    fi
    export EUNOIA_BUILD
...
function lunch()
{
    ...
    then
        ...
        T=$(gettop)
        cd $T > /dev/null
        vendor/os/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product $release
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/os/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
...
. $ANDROID_BUILD_TOP/vendor/os/build/envsetup.sh
